...
export const mapTokenFromStream = (payload: StreamTokenPayload): LaunchToken | undefined => {
  ...
  // Более тщательная обработка числовых значений
  ...
  return {
    ...
    change24h: parseNumber(payload.change24h, 0),
    ...
    network: 'Solana',
    score: parseNumber(payload.holders ?? payload.score, 0),
    ...
    raw: {
      ...
      holders: parseNumber(payload.holders ?? payload.score, 0),
      progress: parseNumber(payload.progress, 0)
    } as RawLaunchToken
  };
};
